...
def create_customer_card(customer_row, segment_korean):
    """고객 카드 생성"""
    # customer_row가 Series인지 dict인지 확인
    if hasattr(customer_row, "get"):
        customer_id = customer_row.get("customer_id", "Unknown")
        total_spent = customer_row.get("total_spent", 0)
        purchase_frequency = customer_row.get("purchase_frequency", 0)
    else:
        customer_id = getattr(customer_row, "customer_id", "Unknown")
        total_spent = getattr(customer_row, "total_spent", 0)
        purchase_frequency = getattr(customer_row, "purchase_frequency", 0)
    ...
    ">
        <h4 style="color: #333; margin: 0 0 10px 0;">고객 {customer_id}</h4>
        <p style="color: #666; margin: 5px 0;"><strong>세그먼트:</strong> {segment_korean}</p>
        <p style="color: #666; margin: 5px 0;"><strong>💰 총 구매금액:</strong> ${total_spent:.2f}</p>
        <p style="color: #666; margin: 5px 0;"><strong>📦 구매 빈도:</strong> {purchase_frequency}회</p>
    </div>
    """
